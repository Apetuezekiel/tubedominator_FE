...
import { actionTemplateFunc } from "./helpers";
...
const Users = ({ userCat, addUserCTA, userPageTitle, userPageTag }) => {
  ...
  useEffect(() => {
    setFetchingUsersData(true);

    const userType = userCat.toLowerCase();
    console.log("userType", userType);
    const fetchData = async (page) => {
      try {
        const result = await fetchUsers(userType, page);
        console.log("usersData result", result.data);
  ...
  const refetchUsersData = async (action) => {
    ...
    try {
      ...
      const result = await fetchUsers(userType, currentPage + 1);
      setUsersData(result.data);
      loaderState(false);
    } catch (error) {
  ...
  const searchAllUsers = async () => {
    // Stop the function if searchAllQuery is empty or null
    if (!searchAllQuery) {
      return null;
    }
    ...
    try {
      const result = await searchUsers(searchAllQuery, userType);
  ...
  const handleSearchAllChange = (event) => {
    const newSearchAllQuery = event.target.value;
    setSearchAllQuery(newSearchAllQuery);

    if (!newSearchAllQuery) {
      // Fetch data again if search query is empty
      setReloadUsersData((prev) => !prev);
    }
  };
          ...
          <div className="w-1/2 flex justify-end py-2">
            <div className="w-full max-w-xs flex items-center p-2 pl-4 pr-4 border border-gray-300 bg-white rounded-full">
              ...
              <AiFillCheckCircle
                ...
                color={searchAllQuery ? "#7438FF" : "gray"}
                size={20}

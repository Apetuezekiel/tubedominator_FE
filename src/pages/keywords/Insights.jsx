    ...
    <section className="w-full z-50">
      ...
      <header>
        <div className="flex items-center justify-between mb-5">
          <span className="mr-3 flex items-center cursor-pointer" onClick={()=> setShowInsights(false)}>
            <BiArrowBack color="#7438FF" className="mr-2" /> Back to list
          </span>
          <span className="text-3xl font-bold mb-2 cursor-pointer" onClick={()=> setShowInsights(false)}>
            <MdCancel color="red"/>
          </span>
        </div>
      ...
      <section className="section1 m-2 mt-14 p-2 px-5 py-10 border-2 bg-white rounded-3xl flex shadow-lg">
        <div className="w-2/6 border-r-2">
          ...
          <div className="mt-8">
            <div className="text-3xl font-semibold">{formatNumberToKMBPlus(dataSet.estimated_views)}</div>
        ...
        <div className="w-4/6 px-8">
          ...
          <div className="mt-8">
            ...
            <span className="text-3xl font-semibold mr-3">{formatNumberToKMBPlus(dataSet.estimated_views)}</span>
            <span className="text-sm">per month</span>
          </div>
          <div className="mt-8">
            <div className="mb-3 text-sm">Last Month</div>
            <div className="flex items-center">
              <span className="text-3xl font-semibold mr-3">{formatNumberToKMBPlus(dataSet.m1)}</span>
              <span
          ...
          <div className="flex justify-between">
            <div className="mt-5 mr-10">
              <div className="flex items-center mt-5 mb-5">
                <span className="mr-2 text-xs text-gray-500 font-semibold">Keyword Difficulty</span>
            ...
            <div className="mt-5 mr-10">
              <div className="flex items-center mt-5 mb-5">
                <span className="mr-2 text-xs text-gray-500 font-semibold">Competitor's Channel Size</span>
                <AiOutlineInfoCircle size={10} className="text-gray-500"/>
              </div>
              <span
              className="rounded-full flex items-center justify-center px-5 py-2 w-30 text-xs"
            ...
            <div className="mt-5 mr-10">
              <div className="flex items-center mt-5 mb-5">
                <span className="mr-2 text-xs text-gray-500 font-semibold">Age of Top Videos</span>
                <AiOutlineInfoCircle size={10} className="text-gray-500"/>
              </div>
              <span
              className="rounded-full flex items-center justify-center px-5 py-2 w-30 text-xs"
          ...
          <div className="mt-8">
            {
              isResultLoaded ? (
                keywordVideosInfo.map((item, index) => {
                  return <div className="mt-5 flex items-top" key={index}>
                  <div>
                    <img
                    ...
                    />
                    <span className="text-xs text-center">{item.length}</span>
                  </div>
                  <div className="ml-2">
                    <div className="text-xs text-gray-800 capitalize break-words">{item.title}</div>
      ...
      {savedIdeasData.related_keywords.map((item, index) => (
        <div key={index} className="m-2">
          <div className="flex rounded-full bg-gray-200 p-2">
            <div className="bg-gray-300 rounded-l-full px-3 py-1 text-xs">{item.string}</div>
            <div className="bg-gray-400 rounded-r-full px-3 py-1 ml-2 text-xs">{formatNumberToKMBPlus(item.volume)}</div>
          </div>
        </div>
      ))}
    </div>
